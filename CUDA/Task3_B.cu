...
{
		char w = *width;
    unsigned int i = (row * w * 4) + (col * 4) + 1;
    return image[i];
}
...
{
		char w = *width;
    unsigned int i = (row * w * 4) + (col * 4) + 2;
    return image[i];
}
...
{
		char w = *width;
    unsigned int i = (row * w * 4) + (col * 4) + 3;
    return image[i];
}
...
int get_time(struct timespec *start, struct timespec *stop,
                   long long int *diff)
{
  long long int in_sec = stop->tv_sec - start->tv_sec;
  long long int in_nano = stop->tv_nsec - start->tv_nsec;
  if (in_nano < 0)
  {
    in_sec--;
    in_nano += 1000000000;
...
int main(void){
  ...
  get_time(&start, &stop, &elasped_time);
